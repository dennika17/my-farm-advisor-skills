...
The data-pipeline includes an offline, single-file **NDVI-based Crop Health Monitoring Dashboard** generator. It produces a self-contained HTML file with embedded Plotly.js, farm/field GeoJSON data, per-field NDVI time series from Sentinel-2 composites, per-field weather-derived GDD and rainfall, and an optional satellite basemap — all inlined so the file works from `file://` with zero runtime external dependencies.

### What it produces

- One HTML file per farm, placed by default at `<farm-dir>/<farm-slug>_crop_health_dashboard.html`
- Interactive Plotly map of field polygons with click-to-toggle selection
- Mean NDVI chart (yearly composite means) with field-year markers
- Rainfall chart (daily bars + cumulative line) with dual Y-axes
- Growing Degree Days chart (cumulative GDD, base 10°C) with frost-date markers
- Custom multi-select dropdowns for fields and years, with synchronized chart axes
- Responsive layout: two-column on desktop, stacked on narrow screens
...
The `--generate-crop-health-dashboard` flag appends dashboard generation as a final pipeline step. It remains backward compatible: dashboards are **not** generated by default.
...
The dashboard reads pre-generated NDVI yearly composites from:

```text
<farm-dir>/fields/<field-id>/derived/features/ndvi_year_YYYY_composite.tif
```

These composites are produced by the `generate_ndvi_composites.py` pipeline step. For each composite:
1. The GeoTIFF is opened with rasterio
2. The field boundary polygon masks the raster via `rasterio.mask.mask()`
3. Mean, standard deviation, and valid pixel count are computed (excluding NaN nodata)
4. Results are embedded in the dashboard as yearly mean NDVI points

Fields without satellite imagery (e.g., missing Sentinel-2/Landsat scenes) are included in the dashboard but labeled as having no NDVI data.

### Weather calculations and units

Same as the Grower Field Weather Dashboard:
- **Growing Degree Days (GDD):**
  - `dailyGdd = max((T2M_MAX + T2M_MIN) / 2 - 10.0, 0)`
  - `cumulativeGdd` is the running total beginning on the last frost date
- **Rainfall:**
...
Same as the Grower Field Weather Dashboard. At generation time, the generator optionally acquires Esri World Imagery tiles, stitches them with Pillow, crops to the buffered farm extent, and base64-encodes the result into the HTML. Use `--no-basemap` to skip tile acquisition and produce a neutral-background dashboard.
...
| `Failed to download Plotly bundle` | Build-time network is required for the first run (or pre-stage `shared/assets/plotly-<version>.min.js`). |
| `Basemap unavailable` | Esri tiles may be unreachable; use `--no-basemap` for a fully offline dashboard. |
| `NDVI field-year combos: 0` | Fields lack pre-generated `derived/features/ndvi_year_*_composite.tif` files. Re-run the pipeline to generate composites. |
...
```text
my-farm-advisor/data-pipeline/src/scripts/reporting/
├── generate_crop_health_dashboard.py   (main generator)
├── dashboard_assets.py                 (extended: NDVI extraction + weather transforms)
...
For the current Illinois fixture, the generated example dashboard is at:
